...
**If contracts are missing but requirements mention API endpoints:**
- Set `status: UNVERIFIED` (cannot fully bind ACs to contracts)
- Add to `concerns[]`: "API contracts missing; test plan is best-effort until interface-designer runs"
- Recommend routing to `interface-designer` via `route_to_agent` in your handoff
- Proceed with what you can derive from requirements (do not block entirely)
